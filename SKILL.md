...
- `/3p --models set <codex|gemini> <low|high> <model>` → call `3PSH models set ...`, print the result, and tell the user to restart Claude Code so PAL MCP reloads the updated reviewer role args. If PAL MCP is managed as a separate process, restart that process instead.
...
3. **Run both reviewers in parallel**:
   - Resolve and install the PAL role names for this run: `codex_role=$(3PSH reviewer-role <run-id> codex)` and `gemini_role=$(3PSH reviewer-role <run-id> gemini)`.
   - Single message with two `mcp__pal__clink` calls — `cli_name=codex, role=<codex_role>` and `cli_name=gemini, role=<gemini_role>`.
